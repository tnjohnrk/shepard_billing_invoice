...
export function getMonthlyReport(year, month) {
  ...
  const invoices = db.prepare('SELECT * FROM invoices WHERE SUBSTR(invoice_date, 1, 7) = ? AND (is_deleted = 0 OR is_deleted IS NULL) ORDER BY invoice_date ASC').all(monthStr);
  const proformas = db.prepare('SELECT * FROM proformas WHERE SUBSTR(proforma_date, 1, 7) = ? AND (is_deleted = 0 OR is_deleted IS NULL) ORDER BY proforma_date ASC').all(monthStr);
